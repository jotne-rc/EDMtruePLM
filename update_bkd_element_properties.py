...
def update_breakdown_element_prop(model: str, node: str, upload_prop_data: dict, repository: str = "TruePLMprojectsRep") -> dict:
    """
    ...
    """
    update_prop_url = f"{plm_info['url']}/api/bkd/prop/{repository}/{model}/{node}"
    print(f"\nPerforming request: {update_prop_url}")

    try:
        response = requests.post(update_prop_url, headers=headers, data=upload_prop_data, timeout=2.0)
...
def main():
    ...
    node = "201863476589"
    upload_result = update_breakdown_element_prop(model, node, upload_prop_data)
    if upload_result:
