...
def document_advanced_search(data_param,model,repository="TruePLMprojectsRep"):
    ...
    """
    doc_search_url = f"{plm_info['url']}/api/dat/a_search/{repository}/{model}"
    print(f"\nPerforming request: {doc_search_url}")

    try:
        response = requests.get(doc_search_url,headers=headers, params=data_param,timeout=2.0)
...
def main():
    ...
    search_result = document_advanced_search(data_param, model)
